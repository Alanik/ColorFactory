var Game = function ( Settings, Cursor, Effects, AnimationManager, Map, PlrObj )
{
			...
			if ( mainPlayer.getCurrentAttackStatus() !== mainPlayer.getAttackStatuses().shooting )
			{
				...
				if ( targetedOpponentObj != null )
				{
					...
					if ( game.shootingPathIsClear( calculatedTiles ) )
					{
						...
						targetedOpponentObj.player.setIsTargeted( true );
					}
	...
	game.moveOtherPlayer = function ( col, row, playerObj, opacityChangeValue )
	{
		...
		var otherPlayerSeatNumber = otherPlayer.getSeatNumber();

		if ( otherPlayer.getIsTargeted )
		{
			otherPlayer.setCurrentMovementStatus( otherPlayer.getMovementStatuses().target );
		}
		else
		{
